...
int8_t SI4463_ReadRxFifoFast(const si4463_t *si4463, uint8_t *msg, const uint16_t msgLen) {
	/* Fast version - minimal CTS wait */
	uint8_t tries = 50;
	while (tries-- && !si4463->IsClearToSend());

	if (!si4463->IsClearToSend()) {
		return SI4463_ERR_NO_HW_CTS;
	}
	...
	si4463->Select();
	si4463->WriteRead(command, cmdBuf, msgLen + 1);
	si4463->Deselect();
...
uint8_t SI4463_GetRxFifoReceivedBytesFast(const si4463_t *si4463) {
	/* Fast version - minimal CTS wait, direct SPI */
	uint8_t tries = 50;
	while (tries-- && !si4463->IsClearToSend());

	/* Send FIFO_INFO command */
	uint8_t cmd[2] = { SI4463_CMD_FIFO_INFO, 0x00 };
	si4463->Select();
	si4463->WriteRead(cmd, NULL, 2);
	si4463->Deselect();

	/* Quick CTS check before read */
	tries = 50;
	while (tries-- && !si4463->IsClearToSend());

	/* Read response: [CTS, RX_FIFO_COUNT, TX_FIFO_SPACE] */
	uint8_t readCmd = SI4463_CMD_READ_CMD_BUF;
	uint8_t resp[4] = {0};
	si4463->Select();
	si4463->WriteRead(&readCmd, NULL, 1);
	si4463->WriteRead(NULL, resp, 4);
	si4463->Deselect();

	/* resp[0] should be CTS (0xFF), resp[1] is RX FIFO count */
	if (resp[0] == 0xFF) {
		return resp[1];
	}
	return 0;
...
int8_t SI4463_TransmitFast(const si4463_t *si4463, const uint8_t *packet, const uint8_t len) {
	/* Fast transmit using GPIO0 TX_STATE indicator for reliable timing */
	int8_t result = SI4463_OK;
	...
	/* Write to FIFO directly */
	uint8_t command[len + 1];
	command[0] = SI4463_CMD_WRITE_TX_FIFO;
	memcpy(&command[1], packet, len);
	si4463->Select();
	si4463->WriteRead(command, NULL, len + 1);
	si4463->Deselect();

	/* Quick CTS check before START_TX */
	tries = 20;
	while (tries-- && !si4463->IsClearToSend()) {}
	...
	uint8_t startCmd[5] = {
		...
		0x00,  /* Stay in READY after TX */
		(len >> 8) & 0x1F,
		len & 0xFF
	};
	si4463->Select();
	si4463->WriteRead(startCmd, NULL, 5);
	si4463->Deselect();

	/* Wait for TX to complete via GPIO0 (TX_STATE indicator)
	 * GPIO0 is HIGH while in TX state, LOW when done */

	/* First, wait for TX to actually start (GPIO0 goes HIGH) */
	uint16_t timeout = 500;
	while (timeout-- && !si4463->IsTxActive()) {
		/* Spin until GPIO0 goes HIGH (TX started) */
	}

	/* Now wait for TX to complete (GPIO0 goes LOW) */
	timeout = 5000;
	while (timeout-- && si4463->IsTxActive()) {
		/* Spin until GPIO0 goes LOW (TX complete) */
	}

	/* Guard delay to ensure RF transmission fully completes */
	volatile uint32_t guard = 14000;
	while (guard--) {
