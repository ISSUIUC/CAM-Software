...
static esp_cam_sensor_device_t s_tvp_sensor;
static esp_cam_sensor_format_t s_tvp_format;

static int tvp_query_support_formats(esp_cam_sensor_device_t *dev, esp_cam_sensor_format_array_t *parry)
{
    return ESP_OK;
}

static int tvp_query_support_capability(esp_cam_sensor_device_t *dev, esp_cam_sensor_capability_t *arg)
{
    return ESP_OK;
}

static int tvp_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    return ESP_OK;
}

static int tvp_get_format(esp_cam_sensor_device_t *dev, esp_cam_sensor_format_t *format)
{
    *format = *(dev->cur_format);
    return ESP_OK;
}

static int tvp_priv_ioctl(esp_cam_sensor_device_t *dev, uint32_t cmd, void *arg)
{
    return ESP_OK;
}

static int tvp_del(esp_cam_sensor_device_t *dev)
{
    return ESP_OK;
}

static const esp_cam_sensor_ops_t tvp_ops = {
    .set_format = tvp_set_format,
    .get_format = tvp_get_format,
    .query_support_formats = tvp_query_support_formats,
    .query_support_capability = tvp_query_support_capability,
    .priv_ioctl = tvp_priv_ioctl,
    .del = tvp_del,
};

static esp_cam_sensor_device_t *tvp5151_create_fake_sensor(void)
{
    memset(&s_tvp_sensor, 0, sizeof(s_tvp_sensor));
    memset(&s_tvp_format, 0, sizeof(s_tvp_format));

    s_tvp_sensor.name = "DVP";
    s_tvp_sensor.ops = &tvp_ops;
    s_tvp_sensor.xclk_pin = GPIO_NUM_NC;
    s_tvp_sensor.reset_pin = GPIO_NUM_NC;
    s_tvp_sensor.pwdn_pin = GPIO_NUM_NC;
    s_tvp_sensor.sensor_port = ESP_CAM_SENSOR_DVP;

    s_tvp_sensor.cur_format = &s_tvp_format;

    s_tvp_format.port = ESP_CAM_SENSOR_DVP;
    s_tvp_format.format = ESP_CAM_SENSOR_PIXFORMAT_YUV422_UYVY;
    s_tvp_format.width = 720;
    s_tvp_format.height = 480;
    s_tvp_format.xclk = 0;
    s_tvp_format.fps = 12;
    s_tvp_format.regs = NULL;
    s_tvp_format.regs_size = 0;
    s_tvp_format.isp_info = NULL;
...
esp_err_t esp_video_init(const esp_video_init_config_t *config)
{
    ...
    if (!dvp_inited && config->dvp != NULL)
    {
        ...
        cam_dev = tvp5151_create_fake_sensor();
        if (!cam_dev)
        {
